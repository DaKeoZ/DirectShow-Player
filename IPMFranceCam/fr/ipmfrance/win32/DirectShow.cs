...
namespace fr.ipmfrance.win32
{
    ...
    public class AMMediaType
    {
        ...
        public static void Init(ref AMMediaType aMMediaType)
        {
            if (((object)aMMediaType) == null)
            {
                aMMediaType = new AMMediaType();
            }
            else
            {
                Free(ref aMMediaType);
            }
            aMMediaType.majorType = Guid.Empty;
            aMMediaType.subType = Guid.Empty;
            aMMediaType.fixedSizeSamples = true;
            aMMediaType.temporalCompression = false;
            aMMediaType.sampleSize = 0;
            aMMediaType.formatType = Guid.Empty;
            aMMediaType.unkPtr = IntPtr.Zero;
            aMMediaType.formatPtr = IntPtr.Zero;
            aMMediaType.formatSize = 0;
        }
